...
from replace_letters_with_position_in_alphabet import alphabet_index
...
class ReplaceLettersWithPositionInAlphabetTestCase(unittest.TestCase):
    def test_replace_each_letter_in_string_with_its_position_in_alphabet(self):
        for txt, expected_value in test_values:
            with self.subTest():
                self.assertEqual(alphabet_index(txt), expected_value)
